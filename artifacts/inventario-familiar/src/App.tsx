...
function printInvoice(sale: Sale, company: { name: string; nit?: string | null; address?: string | null; phone?: string | null }) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Factura Venta #${sale.saleNumber}</title><style>@page{size:80mm auto;margin:2mm 4mm}body{font-family:'Courier New',monospace;font-size:11px;margin:0;padding:0;color:#000}table{width:100%;border-collapse:collapse}td{padding:2px 0}h2{text-align:center;margin:4px 0;font-size:13px}.line{border-top:1px dashed #000;margin:6px 0}.total{font-size:14px;font-weight:bold;text-align:center;margin:8px 0}.footer{text-align:center;margin-top:10px;font-size:9px;color:#555}</style></head><body><div style="text-align:center"><b style="font-size:14px">${companyLines}</b><div class="line"></div><b>FACTURA DE VENTA</b><div>Venta #${sale.saleNumber}</div><div>${dateStr}</div></div><div class="line"></div><table>${items}</table><div class="line"></div><div class="total">TOTAL: ${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(sale.total)}</div><div style="text-align:center;font-size:10px;margin:4px 0">Pago: ${sale.paymentMethod || 'No indicado'}</div>${sale.clientName ? `<div style="text-align:center;font-size:10px">Cliente: ${sale.clientName}${sale.clientPhone ? ` (${sale.clientPhone})` : ''}</div>` : ''}${sale.notes ? `<div style="text-align:center;font-size:10px;margin-top:4px">Nota: ${sale.notes}</div>` : ''}<div class="line"></div><div class="footer">Gracias por su compra</div></body></html>`;
  const w = window.open('', '_blank', 'width=320,height=600');
  if (w) { w.document.write(html); w.document.close(); w.print(); }
}

function SalePage() {
  const products = useListProducts(); const qc = useQueryClient(); const sale = useCreateSale(); const todaySales = useGetSalesReport({ period: 'today' }); const { activeCompany } = useCompany(); const allowNegative = !!activeCompany?.allowNegativeStock; const isPrema = activeCompany?.id === 2; const [items, setItems] = useState<{ productId: number; quantity: number; unitPrice: number }[]>([]); const [selected, setSelected] = useState(''); const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const [quantity, setQuantity] = useState('1'); const [paymentMethod, setPaymentMethod] = useState('Efectivo'); const [notes, setNotes] = useState(''); const [clientName, setClientName] = useState(''); const [clientPhone, setClientPhone] = useState(''); const [saleDate, setSaleDate] = useState(() => toLocalDateString(new Date())); const [error, setError] = useState(''); const [result, setResult] = useState<Sale | null>(null);
  ...
  const submit = () => { if (!items.length) { setError('Agrega al menos un producto para registrar la venta.'); return; } sale.mutate({ data: { items, date: new Date(`${saleDate}T00:00:00-05:00`).toISOString(), paymentMethod, notes: notes.trim() || undefined, clientName: clientName.trim() || undefined, clientPhone: clientPhone.trim() || undefined } }, { onSuccess: (created) => { setResult(created); setItems([]); setNotes(''); setClientName(''); setClientPhone(''); setSaleDate(toLocalDateString(new Date())); qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); qc.invalidateQueries({ queryKey: getListSalesQueryKey() }); qc.invalidateQueries({ queryKey: ['/api/reports/sales'] }); }, onError: () => setError('No se pudo registrar la venta. Revisa las existencias.') }); };
...
function Reports() {
  const [period, setPeriod] = useState<'today' | 'last7' | 'thisMonth' | 'previousMonth' | 'all'>('today'); const [dateFrom, setDateFrom] = useState(''); const [dateTo, setDateTo] = useState(''); const [month, setMonth] = useState('');
