...
import {
  ...
  useAddInventory, useCreateProduct, useCreateSale, useDeleteDemoProducts, useGetDashboard,
  useGetInventoryReport, useGetSalesReport, useListProducts, useListSales,
  useUpdateProduct
} from '@workspace/api-client-react';
import type { Product, Sale, SaleItemInput } from '@workspace/api-client-react';
...
function Button({ children, variant = 'primary', className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'ghost' | 'danger' }) {
  ...
  return <button {...props} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all active:translate-y-px disabled:cursor-not-allowed disabled:opacity-50 ${styles[variant]} ${className}`} />;
}
  ...
  return <div className="min-h-[100dvh] bg-background">
    ...
    <div className="md:pl-[264px]">
      <header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b bg-[hsl(var(--background)/.88)] px-5 backdrop-blur-md sm:px-8">
        <button className="rounded-lg p-2 md:hidden" onClick={() => setMobileOpen(true)} data-testid="button-open-menu"><Menu size={22} /></button>
...
function Dashboard() {
  ...
  return <Shell><PageHeading eyebrow="Resumen del negocio" title="Buenos días." description="Esto es lo que está pasando con tu negocio hoy." action={<Link href="/app/venta" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-5 text-sm font-bold text-[hsl(var(--primary-foreground))] shadow-[0_4px_0_hsl(var(--primary-border))]" data-testid="link-dashboard-sale"><ShoppingCart size={18} /> Registrar venta</Link>} />
    {dashboard.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[1, 2, 3, 4].map((n) => <div key={n} className="h-36 animate-pulse rounded-2xl bg-[hsl(var(--muted))]" />)}</div> :
      dashboard.isError ? <StatusMessage text="No pudimos cargar tu resumen. Revisa tu conexión e inténtalo de nuevo." onRetry={() => dashboard.refetch()} /> :
        <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Productos" value={String(data?.productCount ?? 0)} detail={`${data?.totalUnits ?? 0} unidades en existencia`} icon={Box} /><Metric label="Ventas de hoy" value={money(data?.todaySalesTotal ?? 0)} detail={`${data?.todaySalesCount ?? 0} ventas registradas`} icon={CircleDollarSign} tone="green" /><Metric label="Valor para vender" value={money(data?.inventorySaleValue ?? 0)} detail="Si vendes todo tu inventario" icon={TrendingUp} tone="orange" /><Metric label="Ganancia posible" value={money(data?.potentialProfit ?? 0)} detail="Antes de gastos adicionales" icon={Sparkles} /></div>
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border bg-[hsl(var(--card))] p-6"><div className="flex items-start justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Acciones rápidas</p><h2 className="mt-2 font-display text-2xl font-bold">¿Qué necesitas hacer?</h2></div><ClipboardList className="text-[hsl(var(--primary))]" size={22} /></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/app/productos" className="group flex items-center gap-4 rounded-xl border p-4 transition-colors hover:border-[hsl(var(--primary)/.5)] hover:bg-[hsl(var(--secondary)/.35)]" data-testid="link-quick-products"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><Box size={21} /></span><span className="flex-1"><b className="block text-sm">Ver productos</b><small className="text-xs text-[hsl(var(--muted-foreground))]">Consulta y actualiza existencias</small></span><ArrowRight size={16} className="text-[hsl(var(--muted-foreground))] transition-transform group-hover:translate-x-1" /></Link><Link href="/app/reportes" className="group flex items-center gap-4 rounded-xl border p-4 transition-colors hover:border-[hsl(var(--primary)/.5)] hover:bg-[hsl(var(--secondary)/.35)]" data-testid="link-quick-reports"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[hsl(var(--accent))]"><BarChart3 size={21} /></span><span className="flex-1"><b className="block text-sm">Ver reportes</b><small className="text-xs text-[hsl(var(--muted-foreground))]">Mira cómo va tu negocio</small></span><ArrowRight size={16} className="text-[hsl(var(--muted-foreground))] transition-transform group-hover:translate-x-1" /></Link></div></section>
            <section className="rounded-2xl border bg-[hsl(var(--primary))] p-6 text-[hsl(var(--primary-foreground))]"><div className="flex items-start justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.16em] opacity-70">Inventario</p><h2 className="mt-2 font-display text-2xl font-bold">Para surtir pronto</h2></div><TriangleAlert size={22} /></div><div className="mt-5">{(data?.lowStockProducts || []).length === 0 ? <p className="rounded-xl bg-black/10 p-4 text-sm">Todo está bien surtido por ahora.</p> : <div className="grid gap-2">{data?.lowStockProducts.slice(0, 4).map((p) => <Link key={p.id} href="/app/productos" className="flex items-center gap-3 rounded-xl bg-black/10 p-3 text-sm hover:bg-black/15" data-testid={`link-low-stock-${p.id}`}><span className="flex-1 font-semibold">{p.name}</span><span className="font-mono-app text-xs">{p.stock} / {p.minimumStock}</span><ArrowRight size={15} /></Link>)}</div>}</div><Link href="/app/productos" className="mt-5 inline-flex items-center gap-2 text-sm font-bold underline underline-offset-4" data-testid="link-see-products">Revisar productos <ArrowRight size={15} /></Link></section></div></>}
  </Shell>;
}

function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  const qc = useQueryClient(); const create = useCreateProduct(); const update = useUpdateProduct();
  const [form, setForm] = useState({ name: product?.name || '', cost: String(product?.cost ?? ''), salePrice: String(product?.salePrice ?? ''), stock: '', minimumStock: String(product?.minimumStock ?? 2) });
  const [error, setError] = useState('');
  const isEdit = !!product; const pending = create.isPending || update.isPending;
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!form.name.trim() || Number(form.salePrice) < 0 || (!isEdit && Number(form.stock) < 0)) { setError('Revisa los datos. El nombre y los precios son necesarios.'); return; } setError(''); const done = () => { qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); onClose(); }; if (isEdit) update.mutate({ id: product.id, data: { name: form.name.trim(), cost: Number(form.cost), salePrice: Number(form.salePrice), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo guardar el producto.') }); else create.mutate({ data: { name: form.name.trim(), cost: Number(form.cost), salePrice: Number(form.salePrice), initialStock: Number(form.stock), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo crear el producto.') }); };
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[hsl(var(--foreground)/.45)] p-4" role="dialog" aria-modal="true"><form onSubmit={submit} className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-auto rounded-2xl border bg-[hsl(var(--card))] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--primary))]">{isEdit ? 'Editar producto' : 'Nuevo producto'}</p><h2 className="mt-1 font-display text-3xl font-bold">{isEdit ? product.name : 'Agrega un producto'}</h2></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-close-product-modal"><X size={20} /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Field label="Nombre del producto" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Por ejemplo: Vela de canela" data-testid="input-product-name" /></div><Field label="Costo para ti" type="number" min="0" step="0.01" value={form.cost} onChange={(e) => setForm({ ...form, cost: e.target.value })} placeholder="0.00" data-testid="input-product-cost" /><Field label="Precio de venta" type="number" min="0" step="0.01" value={form.salePrice} onChange={(e) => setForm({ ...form, salePrice: e.target.value })} placeholder="0.00" data-testid="input-product-sale-price" /><Field label={isEdit ? 'Existencia actual' : 'Existencia inicial'} type="number" min="0" value={isEdit ? String(product.stock) : form.stock} disabled={isEdit} onChange={(e) => setForm({ ...form, stock: e.target.value })} placeholder="0" data-testid="input-product-stock" /><Field label="Avisarme cuando queden" type="number" min="0" value={form.minimumStock} onChange={(e) => setForm({ ...form, minimumStock: e.target.value })} placeholder="2" data-testid="input-product-minimum-stock" /></div>{error && <p className="mt-4 rounded-xl bg-[hsl(var(--destructive)/.1)] p-3 text-sm text-[hsl(var(--destructive))]" data-testid="status-product-error">{error}</p>}<div className="mt-7 flex justify-end gap-3"><Button type="button" variant="ghost" onClick={onClose} data-testid="button-cancel-product">Cancelar</Button><Button type="submit" disabled={pending} data-testid="button-save-product">{pending ? 'Guardando…' : isEdit ? 'Guardar cambios' : 'Crear producto'}</Button></div></form></div>;
}
...
function Products() {
  const products = useListProducts(); const qc = useQueryClient(); const deleteDemo = useDeleteDemoProducts(); const [search, setSearch] = useState(''); const [sort, setSort] = useState('name'); const [modal, setModal] = useState<'new' | Product | null>(null); const [inventory, setInventory] = useState<Product | null>(null);
  const removeDemo = () => { if (!window.confirm('¿Borrar los productos de ejemplo? Esto no tocará los productos que hayas creado.')) return; deleteDemo.mutate(undefined, { onSuccess: () => { qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); }, onError: () => window.alert('No se pudieron borrar los datos de ejemplo. Inténtalo nuevamente.') }); };
  const list = useMemo(() => [...(products.data || [])].filter((p) => p.name.toLowerCase().includes(search.toLowerCase())).sort((a, b) => sort === 'stock' ? a.stock - b.stock : sort === 'price' ? b.salePrice - a.salePrice : a.name.localeCompare(b.name)), [products.data, search, sort]);
  return <Shell><PageHeading eyebrow="Catálogo" title="Productos" description="Ten a la mano lo que vendes y cuánto te queda." action={<Button onClick={() => setModal('new')} data-testid="button-new-product"><Plus size={18} /> Nuevo producto</Button>} />
    <div className="mb-5 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search size={18} className="absolute left-3 top-3.5 text-[hsl(var(--muted-foreground))]" /><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar un producto..." className="h-12 w-full rounded-xl border bg-[hsl(var(--card))] pl-10 pr-4 outline-none focus:border-[hsl(var(--primary))]" data-testid="input-search-products" /></label><label className="relative"><select value={sort} onChange={(e) => setSort(e.target.value)} className="h-12 w-full appearance-none rounded-xl border bg-[hsl(var(--card))] pl-4 pr-10 text-sm font-semibold outline-none sm:w-52" data-testid="select-sort-products"><option value="name">Ordenar: nombre</option><option value="stock">Ordenar: menos existencia</option><option value="price">Ordenar: precio mayor</option></select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-4 text-[hsl(var(--muted-foreground))]" /></label></div>
     {products.isLoading ? <div className="grid gap-3">{[1, 2, 3, 4].map((n) => <div key={n} className="h-24 animate-pulse rounded-2xl bg-[hsl(var(--muted))]" />)}</div> : products.isError ? <StatusMessage text="No pudimos cargar los productos." onRetry={() => products.refetch()} /> : list.length === 0 ? <StatusMessage type="empty" text={search ? 'No encontramos productos con ese nombre.' : 'Todavía no tienes productos. Crea el primero para empezar.'} /> : <div className="overflow-hidden rounded-2xl border bg-[hsl(var(--card))]"><div className="hidden grid-cols-[1fr_120px_120px_140px_165px] gap-4 border-b bg-[hsl(var(--muted)/.45)] px-5 py-3 font-mono-app text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] md:grid"><span>Producto</span><span>Existencia</span><span>Precio</span><span>Estado</span><span /></div>{list.map((p) => { const low = p.stock <= p.minimumStock; return <div key={p.id} className="grid gap-4 border-b p-5 last:border-0 md:grid-cols-[1fr_120px_120px_140px_165px] md:items-center md:px-5 md:py-4" data-testid={`row-product-${p.id}`}><div className="flex items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-xl font-mono-app text-xs font-bold ${low ? 'bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]' : 'bg-[hsl(var(--accent))]'}`}>{p.name.slice(0, 2).toUpperCase()}</span><div><p className="font-bold">{p.name}</p><p className="text-xs text-[hsl(var(--muted-foreground))]">Costo {money(p.cost)}</p></div></div><div><span className="font-mono-app text-sm font-bold">{p.stock}</span><span className="ml-1 text-xs text-[hsl(var(--muted-foreground))]">pzas.</span></div><p className="font-mono-app text-sm">{money(p.salePrice)}</p><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-bold ${low ? 'bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]' : 'bg-[hsl(var(--accent)/.7)] text-[hsl(var(--accent-foreground))]'}`}>{low ? 'Por surtir' : 'En existencia'}</span><div className="flex gap-2"><Button variant="secondary" className="min-h-9 px-3 text-xs" onClick={() => setInventory(p)} data-testid={`button-add-inventory-${p.id}`}><PackagePlus size={15} /> Sumar</Button><button onClick={() => setModal(p)} className="grid h-9 w-9 place-items-center rounded-xl border text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" title="Editar producto" data-testid={`button-edit-product-${p.id}`}><Pencil size={15} /></button></div></div>})}</div>}
     <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-dashed bg-[hsl(var(--card)/.55)] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">¿Ya vas a empezar en serio?</p><p className="text-xs text-[hsl(var(--muted-foreground))]">Puedes quitar los productos de ejemplo sin borrar los tuyos.</p></div><Button type="button" variant="ghost" className="text-[hsl(var(--destructive))]" onClick={removeDemo} disabled={deleteDemo.isPending} data-testid="button-delete-demo-products">{deleteDemo.isPending ? 'Borrando…' : 'Borrar datos de ejemplo'}</Button></div>
...
function SalePage() {
  const products = useListProducts(); const qc = useQueryClient(); const sale = useCreateSale(); const [items, setItems] = useState<SaleItemInput[]>([]); const [selected, setSelected] = useState(''); const [quantity, setQuantity] = useState('1'); const [error, setError] = useState(''); const [result, setResult] = useState<Sale | null>(null);
  const chosen = products.data?.filter((p) => items.some((i) => i.productId === p.id)) || [];
  const total = items.reduce((sum, i) => sum + (products.data?.find((p) => p.id === i.productId)?.salePrice || 0) * i.quantity, 0);
  const addItem = () => { const id = Number(selected); const qty = Number(quantity); const p = products.data?.find((x) => x.id === id); if (!p || qty < 1) return; const existing = items.find((i) => i.productId === id); if (existing && existing.quantity + qty > p.stock) { setError('No hay suficientes unidades para esa cantidad.'); return; } setItems(existing ? items.map((i) => i.productId === id ? { ...i, quantity: i.quantity + qty } : i) : [...items, { productId: id, quantity: qty }]); setSelected(''); setQuantity('1'); setError(''); };
  const submit = () => { if (!items.length) { setError('Agrega al menos un producto para registrar la venta.'); return; } sale.mutate({ data: { items } }, { onSuccess: (created) => { setResult(created); setItems([]); qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); qc.invalidateQueries({ queryKey: getListSalesQueryKey() }); }, onError: () => setError('No se pudo registrar la venta. Revisa las existencias.') }); };
  if (result) return <Shell><div className="mx-auto max-w-xl py-10 text-center"><span className="mx-auto grid h-20 w-20 place-items-center rounded-3xl bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]"><Check size={38} /></span><p className="mt-7 font-mono-app text-[11px] font-bold uppercase tracking-[.18em] text-[hsl(var(--primary))]">Venta registrada</p><h1 className="mt-2 font-display text-5xl font-bold">Listo, quedó anotada.</h1><p className="mt-4 text-[hsl(var(--muted-foreground))]">Se actualizaron las existencias de tus productos.</p><div className="mt-8 rounded-2xl border bg-[hsl(var(--card))] p-5 text-left"><div className="flex justify-between border-b pb-4 text-sm"><span className="text-[hsl(var(--muted-foreground))]">Total de la venta</span><strong className="font-mono-app text-xl">{money(result.total)}</strong></div><div className="mt-4 grid gap-2">{result.items.map((item) => <div key={item.productId} className="flex justify-between text-sm"><span>{item.quantity} × {item.productName}</span><span className="font-mono-app">{money(item.subtotal)}</span></div>)}</div></div><div className="mt-7 flex justify-center gap-3"><Button variant="secondary" onClick={() => setResult(null)} data-testid="button-new-sale">Registrar otra</Button><Link href="/app" className="inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-bold text-[hsl(var(--primary))]" data-testid="link-sale-dashboard">Volver al inicio <ArrowRight size={16} /></Link></div></div></Shell>;
  return <Shell><PageHeading eyebrow="Caja" title="Registrar venta" description="Agrega lo que se llevó tu cliente y confirma al final." /><div className="grid gap-6 lg:grid-cols-[1fr_370px]"><section className="rounded-2xl border bg-[hsl(var(--card))] p-5 sm:p-7"><div className="grid gap-4 sm:grid-cols-[1fr_150px_auto] sm:items-end"><label className="grid gap-1.5 text-sm font-semibold">Producto<select value={selected} onChange={(e) => setSelected(e.target.value)} className="h-12 rounded-xl border bg-[hsl(var(--background))] px-3 outline-none focus:border-[hsl(var(--primary))]" data-testid="select-sale-product"><option value="">Elige un producto</option>{(products.data || []).filter((p) => p.stock > 0).map((p) => <option key={p.id} value={p.id}>{p.name} · {p.stock} disponibles</option>)}</select></label><Field label="Cantidad" type="number" min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} data-testid="input-sale-quantity" /><Button type="button" onClick={addItem} className="sm:mb-0" data-testid="button-add-sale-item"><Plus size={18} /> Agregar</Button></div>{error && <p className="mt-4 rounded-xl bg-[hsl(var(--destructive)/.1)] p-3 text-sm text-[hsl(var(--destructive))]" data-testid="status-sale-error">{error}</p>}<div className="mt-8 border-t pt-5"><p className="mb-3 font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Productos de esta venta</p>{!items.length ? <div className="grid min-h-40 place-items-center rounded-xl border border-dashed p-5 text-center"><div><ShoppingBasket size={25} className="mx-auto text-[hsl(var(--muted-foreground))]" /><p className="mt-3 text-sm text-[hsl(var(--muted-foreground))]">Aún no hay productos.<br />Elige uno arriba para comenzar.</p></div></div> : <div className="grid gap-2">{items.map((item) => { const p = products.data?.find((x) => x.id === item.productId); return <div key={item.productId} className="flex items-center gap-3 rounded-xl bg-[hsl(var(--muted)/.55)] p-3" data-testid={`row-sale-item-${item.productId}`}><span className="flex-1 text-sm font-bold">{p?.name}</span><span className="font-mono-app text-xs">{item.quantity} × {money(p?.salePrice || 0)}</span><button onClick={() => setItems(items.filter((x) => x.productId !== item.productId))} className="rounded-lg p-1.5 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--card))]" data-testid={`button-remove-sale-item-${item.productId}`}><X size={15} /></button></div>})}</div>}</div></section><aside className="h-fit rounded-2xl bg-[hsl(var(--sidebar))] p-6 text-[hsl(var(--sidebar-foreground))]"><p className="font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--sidebar-primary))]">Resumen</p><h2 className="mt-2 font-display text-2xl font-bold">Total a cobrar</h2><p className="mt-6 font-mono-app text-5xl font-bold text-[hsl(var(--sidebar-primary))]">{money(total)}</p><p className="mt-2 text-sm text-[hsl(var(--sidebar-foreground)/.6)]">{items.reduce((sum, i) => sum + i.quantity, 0)} productos en total</p><Button className="mt-7 w-full bg-[hsl(var(--sidebar-primary))] text-[hsl(var(--sidebar-primary-foreground))]" onClick={submit} disabled={sale.isPending || !items.length} data-testid="button-confirm-sale">{sale.isPending ? 'Registrando…' : 'Confirmar venta'} <ArrowRight size={17} /></Button><p className="mt-4 text-center text-xs text-[hsl(var(--sidebar-foreground)/.5)]">Al confirmar, se descuentan las existencias automáticamente.</p></aside></div></Shell>;
}
