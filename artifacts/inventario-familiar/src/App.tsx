...
function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  const allProducts = useListProducts(); const { activeCompany } = useCompany();
  ...
  const suppliers = useMemo(() => supplierOptions(activeCompany?.id, (allProducts.data || []).map((p) => p.supplier)), [allProducts.data, activeCompany]);
...
function Products() {
  ...
  const suppliers = useMemo(() => supplierOptions(activeCompany?.id, (products.data || []).map((p) => p.supplier)), [products.data, activeCompany]);
...
function PurchaseModal({ onClose }: { onClose: () => void }) {
  const products = useListProducts(); const qc = useQueryClient(); const create = useCreatePurchase(); const { activeCompany } = useCompany();
