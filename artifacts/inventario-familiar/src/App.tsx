...
function printInvoice(sale: Sale, company: { name: string; nit?: string | null; address?: string | null; phone?: string | null }) {
  ...
  const invoiceName = company.name === 'Suplementos Tere' ? 'Tienda Natural Tere' : company.name === 'Tienda Natural Prema' ? 'Tienda Natural Prema' : company.name;
  const companyLines = [invoiceName, company.nit ? `NIT. ${company.nit}` : '', company.address, company.phone ? `Tel: ${company.phone}` : ''].filter(Boolean).join('<br>');
