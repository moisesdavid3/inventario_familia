...
import {
  ...
  ShoppingCart, Sparkles, Trash2, TriangleAlert, TrendingUp, Wallet, X
} from 'lucide-react';
...
import {
  ...
  useAddInventory, useCreateProduct, useCreateSale, useDeleteProduct,
  useGetDashboard, useGetInventoryReport, useGetSalesReport, useListProducts, useListSales,
...
function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ name: product?.name || '', brand: product?.brand || '', cost: String(product?.cost ?? ''), salePrice: String(product?.salePrice ?? ''), stock: '', minimumStock: String(product?.minimumStock ?? 2) });
  const [error, setError] = useState('');
  const [confirming, setConfirming] = useState(false);
  const isEdit = !!product; const pending = create.isPending || update.isPending || del.isPending;
  const brands = useMemo(() => Array.from(new Set((allProducts.data || []).map((p) => p.brand).filter((b): b is string => !!b))).sort((a, b) => a.localeCompare(b)), [allProducts.data]);
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!form.name.trim() || Number(form.salePrice) < 0 || (!isEdit && Number(form.stock) < 0)) { setError('Revisa los datos. El nombre y los precios son necesarios.'); return; } setError(''); const done = () => { qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); onClose(); }; const brand = form.brand.trim() || undefined; if (isEdit) update.mutate({ id: product.id, data: { name: form.name.trim(), brand, cost: Number(form.cost), salePrice: Number(form.salePrice), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo guardar el producto.') }); else create.mutate({ data: { name: form.name.trim(), brand, cost: Number(form.cost), salePrice: Number(form.salePrice), initialStock: Number(form.stock), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo crear el producto.') }); };
...
function Products() {
  const products = useListProducts(); const qc = useQueryClient(); const [search, setSearch] = useState(''); const [sort, setSort] = useState('name'); const [brand, setBrand] = useState('all'); const [status, setStatus] = useState('all'); const [modal, setModal] = useState<'new' | Product | null>(null); const [inventory, setInventory] = useState<Product | null>(null);
  const brands = useMemo(() => Array.from(new Set((products.data || []).map((p) => p.brand).filter((b): b is string => !!b))).sort((a, b) => a.localeCompare(b)), [products.data]);
  const list = useMemo(() => [...(products.data || [])].filter((p) => p.name.toLowerCase().includes(search.toLowerCase()) && (brand === 'all' || p.brand === brand) && (status === 'all' || (status === 'low' ? p.stock <= p.minimumStock : p.stock > p.minimumStock))).sort((a, b) => sort === 'stock' ? a.stock - b.stock : sort === 'price' ? b.salePrice - a.salePrice : a.name.localeCompare(b.name)), [products.data, search, sort, brand, status]);
  return <Shell><PageHeading eyebrow="Catálogo" title="Productos" action={<Button onClick={() => setModal('new')} className="min-h-[28px] px-2.5 text-sm" data-testid="button-new-product"><Plus size={14} /> Nuevo producto</Button>} />
    <div className="mb-5 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search size={18} className="absolute left-3 top-3.5 text-[hsl(var(--muted-foreground))]" /><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar un producto..." className="h-12 w-full rounded-xl border bg-[hsl(var(--card))] pl-10 pr-4 outline-none focus:border-[hsl(var(--primary))]" data-testid="input-search-products" /></label><label className="relative"><select value={brand} onChange={(e) => setBrand(e.target.value)} className="h-12 w-full appearance-none rounded-xl border bg-[hsl(var(--card))] pl-4 pr-10 text-sm font-semibold outline-none sm:w-44" data-testid="select-brand-products"><option value="all">Marca: todas</option>{brands.map((b) => <option key={b} value={b}>Marca: {b}</option>)}</select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-4 text-[hsl(var(--muted-foreground))]" /></label><label className="relative"><select value={status} onChange={(e) => setStatus(e.target.value)} className="h-12 w-full appearance-none rounded-xl border bg-[hsl(var(--card))] pl-4 pr-10 text-sm font-semibold outline-none sm:w-40" data-testid="select-status-products"><option value="all">Estado: todos</option><option value="ok">Estado: en existencia</option><option value="low">Estado: por surtir</option></select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-4 text-[hsl(var(--muted-foreground))]" /></label><label className="relative"><select value={sort} onChange={(e) => setSort(e.target.value)} className="h-12 w-full appearance-none rounded-xl border bg-[hsl(var(--card))] pl-4 pr-10 text-sm font-semibold outline-none sm:w-48" data-testid="select-sort-products"><option value="name">Ordenar: nombre</option><option value="stock">Ordenar: menos existencia</option><option value="price">Ordenar: precio mayor</option></select><ChevronDown size={16} className="pointer-events-none absolute right-3 top-4 text-[hsl(var(--muted-foreground))]" /></label>     </div>
     {!products.isLoading && !products.isError && <p className="mb-3 text-sm text-[hsl(var(--muted-foreground))]" data-testid="text-product-count">{list.length} {list.length === 1 ? 'producto' : 'productos'}{search || brand !== 'all' || status !== 'all' ? ` de ${(products.data || []).length}` : ''}</p>}
     {products.isLoading ? <div className="grid gap-3">{[1, 2, 3, 4].map((n) => <div key={n} className="h-24 animate-pulse rounded-2xl bg-[hsl(var(--muted))]" />)}</div> :  products.isError ? <StatusMessage text="No pudimos cargar los productos." onRetry={() => products.refetch()} /> : list.length === 0 ? <StatusMessage type="empty" text={search || brand !== 'all' || status !== 'all' ? 'No encontramos productos con esos filtros.' : 'Todavía no tienes productos. Crea el primero para empezar.'} /> : <div className="overflow-hidden rounded-2xl border bg-[hsl(var(--card))]"><div className="hidden grid-cols-[1fr_110px_110px_130px_150px] gap-4 border-b bg-[hsl(var(--muted)/.45)] px-5 py-3 font-mono-app text-sm font-bold uppercase tracking-wider text-[hsl(var(--muted-foreground))] md:grid"><span>Producto</span><span>Inventario</span><span>Precio</span><span>Estado</span><span /></div>{list.map((p) => {
        ...
        return <div key={p.id} className="border-b p-5 last:border-0 md:grid md:grid-cols-[1fr_110px_110px_130px_150px] md:items-center md:gap-4 md:px-5 md:py-4" data-testid={`row-product-${p.id}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {p.brand && <p className="whitespace-normal break-words text-sm font-semibold text-[hsl(var(--primary))] md:truncate">{p.brand}</p>}
              <p className="text-xs text-[hsl(var(--muted-foreground))]">Costo {money(p.cost)}</p>
            </div>
            <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold md:hidden ${badgeClass}`}>{low ? 'Por surtir' : 'En stock'}</span>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 md:hidden">
          ...
          <span className={`hidden w-fit rounded-full px-2.5 py-1 text-xs font-bold md:block ${badgeClass}`}>{low ? 'Por surtir' : 'En existencia'}</span>
...
function SalePage() {
  const products = useListProducts(); const qc = useQueryClient(); const sale = useCreateSale(); const [items, setItems] = useState<SaleItemInput[]>([]); const [selected, setSelected] = useState(''); const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const [quantity, setQuantity] = useState('1'); const [error, setError] = useState(''); const [result, setResult] = useState<Sale | null>(null);
  ...
  const matches = useMemo(() => (products.data || []).filter((p) => p.stock > 0 && (p.name.toLowerCase().includes(query.trim().toLowerCase()) || (p.brand || '').toLowerCase().includes(query.trim().toLowerCase()))), [products.data, query]);
  const addItem = () => { const id = Number(selected); const qty = Number(quantity); const p = products.data?.find((x) => x.id === id); if (!p || qty < 1) return; const existing = items.find((i) => i.productId === id); const currentInCart = existing?.quantity || 0; if (currentInCart + qty > p.stock) { setError(`No hay suficientes unidades de ${p.name}. Solo quedan ${p.stock}.`); return; } setItems(existing ? items.map((i) => i.productId === id ? { ...i, quantity: i.quantity + qty } : i) : [...items, { productId: id, quantity: qty }]); setSelected(''); setQuery(''); setQuantity('1'); setError(''); };
