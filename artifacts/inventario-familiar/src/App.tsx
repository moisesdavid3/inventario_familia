...
import type { Client, CreditPayment, ManualCredit, PaymentActivity, Product, Purchase, PurchaseImportResult, PurchaseInput, Sale } from '@workspace/api-client-react';
...
function NewSupplierModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setError('');
    create.mutate({ data: { name: n } }, {
      onSuccess: () => { qc.invalidateQueries({ queryKey: getListSuppliersQueryKey() }); qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); onClose(); },
      onError: () => setError('No se pudo crear el proveedor.'),
    });
  };
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[hsl(var(--foreground)/.45)] p-4" role="dialog" aria-modal="true"><form onSubmit={submit} className="w-full max-w-md rounded-2xl border bg-[hsl(var(--card))] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.16em] text-[hsl(var(--primary))]">Proveedores</p><h2 className="mt-1 font-display text-3xl font-bold">Nuevo proveedor</h2></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-close-new-supplier"><X size={20} /></button></div><div className="mt-6 grid gap-4"><Field label="Nombre del proveedor" value={name} onChange={(e) => setName(e.target.value)} placeholder="Por ejemplo: Bioessens" autoFocus data-testid="input-new-supplier-name" /></div>{error && <p className="mt-4 text-sm text-[hsl(var(--destructive))]" data-testid="status-new-supplier-error">{error}</p>}<div className="mt-7 flex justify-end gap-3"><Button type="button" variant="ghost" onClick={onClose} data-testid="button-cancel-new-supplier">Cancelar</Button><Button type="submit" disabled={create.isPending} data-testid="button-save-new-supplier">{create.isPending ? 'Guardando…' : 'Crear proveedor'}</Button></div></form></div>;
}
...
function Purchases() {
  ...
  return <Shell><PageHeading eyebrow="Surtir inventario" title="Compras" description="Registra las entradas de mercancía y el stock se actualiza solo." action={<><Button variant="secondary" className="mr-2" onClick={() => setSupplierModal(true)} data-testid="button-new-supplier-purchases"><PackagePlus size={16} /> Nuevo proveedor</Button><Button variant="secondary" className="mr-2" onClick={() => setImportModal(true)} data-testid="button-import-purchases"><Upload size={16} /> Importar CSV</Button><Button onClick={() => setModal(true)} data-testid="button-new-purchase"><Plus size={16} /> Nueva compra</Button></>} />
    ...
    {modal && <PurchaseModal onClose={() => setModal(false)} />}{importModal && <ImportModal onClose={() => setImportModal(false)} />}{supplierModal && <NewSupplierModal onClose={() => setSupplierModal(false)} />}
