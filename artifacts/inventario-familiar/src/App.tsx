...
function Reports() {
  ...
  const inventory = useGetInventoryReport({ filter }); const sales = useGetSalesReport({ period }); const tabs = [{ value: 'all', label: 'Todo' }, { value: 'low', label: 'Por surtir' }, { value: 'empty', label: 'Agotados' }]; const qc = useQueryClient(); const deleteSale = useDeleteSale(); const { activeCompany } = useCompany(); const isPrema = activeCompany?.id === 2; const [confirmSale, setConfirmSale] = useState<number | null>(null);
