...
function printInvoice(sale: Sale, company: { name: string; nit?: string | null; address?: string | null; phone?: string | null }) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Factura Venta #${sale.saleNumber}</title><style>@page{size:80mm auto;margin:2mm 4mm}body{font-family:'Courier New',monospace;font-size:9px;margin:0;padding:0;color:#000}table{width:100%;border-collapse:collapse}td{padding:2px 0}h2{text-align:center;margin:4px 0;font-size:11px}.line{border-top:1px dashed #000;margin:5px 0}.total{font-size:12px;font-weight:bold;text-align:center;margin:6px 0}.footer{text-align:center;margin-top:8px;font-size:8px;color:#555}</style></head><body><div style="text-align:center"><b style="font-size:11px">${companyLines}</b><div class="line"></div><b>FACTURA DE VENTA</b><div>Venta #${sale.saleNumber}</div><div>${dateStr}</div></div><div class="line"></div><table>${items}</table><div class="line"></div><div class="total">TOTAL: ${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(sale.total)}</div><div style="text-align:center;font-size:9px;margin:3px 0">Pago: ${sale.paymentMethod || 'No indicado'}</div>${sale.clientName ? `<div style="text-align:center;font-size:9px">Cliente: ${sale.clientName}${sale.clientPhone ? ` (${sale.clientPhone})` : ''}</div>` : ''}${sale.notes ? `<div style="text-align:center;font-size:9px;margin-top:3px">Nota: ${sale.notes}</div>` : ''}<div class="line"></div><div class="footer">Gracias por su compra</div></body></html>`;
