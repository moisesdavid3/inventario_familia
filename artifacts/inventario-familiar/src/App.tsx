...
function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ name: product?.name || '', supplier: product?.supplier || '', category: product?.category || '', cost: String(product?.cost ?? ''), salePrice: String(product?.salePrice ?? ''), stock: '', minimumStock: String(product?.minimumStock ?? 2) });
  const [error, setError] = useState('');
  const [confirming, setConfirming] = useState(false);
  const isEdit = !!product; const pending = create.isPending || update.isPending || del.isPending;
  const suppliers = useMemo(() => Array.from(new Set([...PROVIDERS, ...(allProducts.data || []).map((p) => p.supplier).filter((s): s is string => !!s)])).sort((a, b) => a.localeCompare(b)), [allProducts.data]);
...
function Products() {
  ...
  const suppliers = useMemo(() => Array.from(new Set([...PROVIDERS, ...(products.data || []).map((p) => p.supplier).filter((s): s is string => !!s)])).sort((a, b) => a.localeCompare(b)), [products.data]);
...
function SalePage() {
  const products = useListProducts(); const qc = useQueryClient(); const sale = useCreateSale(); const { activeCompany } = useCompany(); const allowNegative = !!activeCompany?.allowNegativeStock; const [items, setItems] = useState<{ productId: number; quantity: number; unitPrice: number }[]>([]); const [selected, setSelected] = useState(''); const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const [quantity, setQuantity] = useState('1'); const [error, setError] = useState(''); const [result, setResult] = useState<Sale | null>(null);
