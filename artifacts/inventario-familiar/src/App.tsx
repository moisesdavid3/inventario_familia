...
function SalePage() {
  ...
  const submit = () => { if (!items.length) { setError('Agrega al menos un producto para registrar la venta.'); return; } sale.mutate({ data: { items, date: (() => { const now = new Date(); const [y, m, d] = saleDate.split('-').map(Number); const local = new Date(y, m - 1, d, now.getHours(), now.getMinutes(), now.getSeconds()); return local.toISOString(); })(), paymentMethod, notes: notes.trim() || undefined, clientName: clientName.trim() || undefined, clientPhone: clientPhone.trim() || undefined } }, { onSuccess: (created) => { setResult(created); setItems([]); setNotes(''); setClientName(''); setClientPhone(''); setSaleDate(toLocalDateString(new Date())); qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); qc.invalidateQueries({ queryKey: getListSalesQueryKey() }); qc.invalidateQueries({ queryKey: ['/api/reports/sales'] }); }, onError: () => setError('No se pudo registrar la venta. Revisa las existencias.') }); };
