...
import {
  ...
  useAddInventory, useCreateProduct, useCreateSale, useDeleteProduct, useDeleteSale,
...
function ProductModal({ product, onClose }: { product?: Product; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ name: product?.name || '', brand: product?.brand || '', category: product?.category || '', cost: String(product?.cost ?? ''), salePrice: String(product?.salePrice ?? ''), stock: '', minimumStock: String(product?.minimumStock ?? 2) });
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!form.name.trim() || Number(form.salePrice) < 0 || (!isEdit && Number(form.stock) < 0)) { setError('Revisa los datos. El nombre y los precios son necesarios.'); return; } setError(''); const done = () => { qc.invalidateQueries({ queryKey: getListProductsQueryKey() }); qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); onClose(); }; const brand = form.brand.trim() || undefined; const category = form.category.trim() || undefined; if (isEdit) update.mutate({ id: product.id, data: { name: form.name.trim(), brand, category, cost: Number(form.cost), salePrice: Number(form.salePrice), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo guardar el producto.') }); else create.mutate({ data: { name: form.name.trim(), brand, category, cost: Number(form.cost), salePrice: Number(form.salePrice), initialStock: Number(form.stock), minimumStock: Number(form.minimumStock) } }, { onSuccess: done, onError: () => setError('No se pudo crear el producto.') }); };
...
function Products() {
  const products = useListProducts(); const qc = useQueryClient(); const [search, setSearch] = useState(''); const [sort, setSort] = useState('name'); const [brand, setBrand] = useState('all'); const [category, setCategory] = useState('all'); const [status, setStatus] = useState('all'); const [modal, setModal] = useState<'new' | Product | null>(null); const [inventory, setInventory] = useState<Product | null>(null);
  ...
  const list = useMemo(() => [...(products.data || [])].filter((p) => p.name.toLowerCase().includes(search.toLowerCase()) && (brand === 'all' || p.brand === brand) && (category === 'all' || p.category === category) && (status === 'all' || (status === 'low' ? p.stock <= p.minimumStock : p.stock > p.minimumStock))).sort((a, b) => sort === 'stock' ? a.stock - b.stock : sort === 'price' ? b.salePrice - a.salePrice : a.name.localeCompare(b.name)), [products.data, search, sort, brand, category, status]);
