...
function Reports() {
  const [period, setPeriod] = useState<'today' | 'last7' | 'thisMonth' | 'previousMonth' | 'all'>('today'); const [day, setDay] = useState('');
  const salesParams = day ? { from: new Date(day + 'T00:00:00-05:00').toISOString(), to: new Date(day + 'T23:59:59.999-05:00').toISOString() } : { period }; const inventory = useGetInventoryReport({ filter: 'all' }); const sales = useGetSalesReport(salesParams); const qc = useQueryClient(); const deleteSale = useDeleteSale(); const { activeCompany } = useCompany(); const isPrema = activeCompany?.id === 2; const [confirmSale, setConfirmSale] = useState<number | null>(null);
