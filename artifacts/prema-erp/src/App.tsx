...
function Products() {
  ...
  return <Shell><PageHeading eyebrow="Catálogo" title="Inventario" action={<div className="flex items-center justify-end gap-2"><Button variant="secondary" onClick={() => setStockoutModal(true)} className="min-h-[28px] px-2.5 text-sm" data-testid="button-open-stockout"><TriangleAlert size={14} /> Bajas</Button><Button onClick={() => setModal('new')} className="min-h-[28px] px-2.5 text-sm" data-testid="button-new-product"><Plus size={14} /> Nuevo producto</Button></div>} />
    ...
    {modal && <ProductModal product={modal === 'new' ? undefined : modal} onClose={() => setModal(null)} />}{inventory && <InventoryModal product={inventory} onClose={() => setInventory(null)} />}{stockoutModal && <StockoutModal onClose={() => setStockoutModal(false)} />}
  </Shell>;
}
